...
export const PatientTableWithSearch = ({rows, columns, pageSize, mapFiledToHeb}: TableWithSearchProps) => {
    ...
    const requestSearchName = (searchValue: string)=>{
        requestSearch((row)=>(row.firstName.toLowerCase().includes(searchValue.toLowerCase())
...
const PatientTable = ({rows, columns, pageSize, mapFiledToHeb}: TableWithSearchProps) => {
    ...
    const columnsDef: GridColDef[] = columns.map(column => {
        if (column == "id")
            return {field: column, width: 120, headerName: mapFiledToHeb[column], hide: true}
        if (column == MEDICAL_FILE)
            return {field: column, width: 120, headerName: mapFiledToHeb[column], renderCell: BUTTON}
